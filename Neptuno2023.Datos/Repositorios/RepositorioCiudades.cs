...
namespace Neptuno2023.Datos.Repositorios
{
    public class RepositorioCiudades : IRepositorioCiudades
    {
        ...
        public List<CiudadDto> GetComprasDto()
        {
            try
            {
                ...
                {
                    conn.Open();
                    string selectQuery = "SELECT CiudadId, NombreCiudad, PaisId FROM Ciudades ORDER BY NombreCiudad";
                    using (var comando = new SqlCommand(selectQuery, conn))
                    {
                        using (var reader = comando.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                var ciudades = ConstruirCiudadDto(reader);
                                lista.Add((CiudadDto)ciudades);
                            }
                        }
                    }
                }
        ...
        private object ConstruirCiudadDto(SqlDataReader reader)
        {
            return new CiudadDto()
            {
                
                NombreCiudad = reader.GetString(0),
